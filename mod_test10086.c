...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_test10086_shutdown);
SWITCH_MODULE_LOAD_FUNCTION(mod_test10086_load);
SWITCH_MODULE_DEFINITION(mod_test10086, mod_test10086_load, mod_test10086_shutdown, NULL);
...
typedef struct test10086_session_s {
...
{
	test10086_session_t *test10086_t = user_data;
	switch_core_session_t *session = test10086_t->session;
              	...
              	if (voicecount) {
					...
					//    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "voicecount is %s \n",intvoicecount);
						...
						if(intvoicecount>10){
							...
							// switch_core_session_hangup_state(test10086_t->session,SWITCH_FALSE);
						}
              	}
				 else{
    ...
    //     switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod test10086 aload! %d \n",cont->default_expires);
    // }else{
 ...
 */
SWITCH_STANDARD_APP(test10086_start_app)
{
	switch_channel_t *channel = switch_core_session_get_channel(session);
	...
	/* Add media bug */
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Starting test10086\n");
